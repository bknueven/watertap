...
class TestUltraFiltrationZO_w_default_removal:
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(0.01193, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].flow_vol
        )
        assert pytest.approx(58.6751, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["eeq"]
        )
        assert pytest.approx(58.6751, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["toc"]
        )
        assert pytest.approx(2.51465, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["tss"]
        )
        assert pytest.approx(8.3822e-3, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["cryptosporidium"]
        )
        assert pytest.approx(83.8216, rel=1e-5) == value(
        ...
        assert pytest.approx(12.49263, abs=1e-5) == value(model.fs.unit.electricity[0])
...
def test_costing():
    ...
    m.fs.unit1 = UltraFiltrationZO(property_package=m.fs.params, database=m.db)
    ...
    assert isinstance(m.fs.costing.ultra_filtration.capital_a_parameter, Var)
    assert isinstance(m.fs.costing.ultra_filtration.capital_b_parameter, Var)
