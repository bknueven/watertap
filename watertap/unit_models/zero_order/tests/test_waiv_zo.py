...
class TestWAIVZO_w_default_removal:
    ...
    def test_solution(self, model):
        assert pytest.approx(1.002, rel=1e-5) == value(
            model.fs.unit.properties_in[0].flow_vol
        )
        assert pytest.approx(0.998, rel=1e-5) == value(
            model.fs.unit.properties_in[0].conc_mass_comp["tds"]
        )
        assert pytest.approx(0.998, rel=1e-5) == value(
            model.fs.unit.properties_in[0].conc_mass_comp["foo"]
        )
        assert pytest.approx(0.0021, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].flow_vol
        )
        assert pytest.approx(476.19, rel=1e-5) == value(
...
def test_costing():
    ...
    m.fs.unit1 = WAIVZO(property_package=m.fs.params, database=m.db)
    ...
    assert isinstance(m.fs.costing.waiv.capital_a_parameter, Var)
    assert isinstance(m.fs.costing.waiv.capital_b_parameter, Var)
